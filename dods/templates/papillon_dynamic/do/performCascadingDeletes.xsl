...
<xsl:template name="performCascadingDeletes">
<xsl:if test="dodst:compareReferrerDoName(/TABLE/PACKAGE,/TABLE/CLASS_NAME,@do_name) = 'true'">
<xsl:if test="/TABLE/DELETE_CASCADES='false'">    }

    // perform cascading delete on referring table
    private void _performCascadingDelete_<xsl:value-of select="dodst:getArrayDO(../@name,@name)"/>(DBTransaction dbt) throws SQLException, DatabaseManagerException, DataObjectException, QueryException, RefAssertionException {
