...
if (!$ARGV[0]) {print "usage: $0 \"la phrase à traduire (en UTF-8 !)\" [lang=eng|fra]\n";exit};
...
foreach my $mot (split(/ /,$ARGV[0])) {
	my $lemme = get('http://papillon.imag.fr/papillon/api/DELA/eng/cdm-headword-variant/' . $mot . '/cdm-headword/?strategy=EQUAL&count=1');
