...
import fr.imag.clips.papillon.presentation.xhtmlmotamot.orig.LinkerSearchFormXHTML;
import fr.imag.clips.papillon.presentation.xhtmlmotamot.orig.LinkerResultListXHTML;
 ...
 * @created    July 6, 2005
 */
public class MotamotLinker extends LinkerBasePO {
			...
			for (int i=0; i < parameters.dictionaries.length; i++) {
				...
				if (dictName != null && dictName != "") {
					...
					for (Iterator iter = VolumesFactory.getVolumesArray(dict.getName(), parameters.sourceLang, null).iterator(); iter.hasNext();)
					{
						...
						results.addAll(VolumeEntriesFactory.getVolumeEntriesVector(dict, myVolume, keys, null, null, 0, 0));
					}
				}
			}
		}
		else if (parameters.volumes.length>0) {
			for (int i=0; i < parameters.volumes.length; i++) {
